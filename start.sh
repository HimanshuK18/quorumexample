...
echo "--bootnodes value     Comma separated enode URLs for P2P discovery bootstrap"
echo "--networkid value     Network identifier (integer, 0=Olympic (disused), 1=Frontier, 2=Morden (disused), 3=Ropsten) (default: 1)"
echo " --rpc                Enable the HTTP-RPC server"
echo "--rpcaddr value        HTTP-RPC server listening interface (default: localhost)"
echo "--rpcapi value         API's offered over the HTTP-RPC interface (default: eth,net,web3)"
echo "--rpcport value        HTTP-RPC server listening port (default: 8545)"
echo  "--port value          Network listening port (default: 30303)"
...
echo " --nodekeyhex value    P2P node key as hex (for testing)"
echo "wait for bootnode to start..."
sleep 6

echo "[*] Starting node 1 the fisrt node"
PRIVATE_CONFIG=tm1.conf nohup geth --datadir qdata/dd1 $GLOBAL_ARGS --rpcport 22000 --port 21000 --unlock "0xed9d02e382b34818e88b88a309c7fe71e65f419d" --password passwords.txt 2>>qdata/logs/1.log &
echo "--unlock value    Comma separated list of accounts to unlock"
