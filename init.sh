...
echo "[*] Cleaning up temporary data directories and deleting"
rm -rf qdata
mkdir -p qdata/logs
...
# copy the file key1 to the new created folder
geth --datadir qdata/dd1 init genesis.json
# set the data directore for node dd1,Data directory for the databases and keystore and also Bootstrap and initialize a new genesis block
# the genesis file used is in the folder 7nodes
